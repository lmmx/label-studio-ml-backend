...
def get_args():
    ...
    parser_deploy = subparsers.add_parser(
        "deploy", help="Deploy Label Studio", parents=[root_parser]
    )
    ...
    parser_deploy.add_argument(
        "--gcp-project-id", dest="gcp_project", help="GCP project ID"
    )
...
def deploy_to_gcp(args):
    ...
    auth_token = subprocess.check_output(
        " ".join(["gcloud", "auth", "print-identity-token"]), shell=True
    )
    if not auth_token:
        raise PermissionError(
            "You are not authentificated in gcloud! Please run gcloud auth login."
        )
    # configurate project
    subprocess.check_output(
        " ".join(["gcloud", "config", "set", "project", project_id]), shell=True
    )
    # deploy service
    subprocess.check_output(
        " ".join(
