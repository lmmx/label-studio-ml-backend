...
class BertClassifier(LabelStudioMLBase):
    def __init__(
        ...
        **kwargs
        ...
        if not self.train_output:
            ...
            self.reset_model(
                "bert-base-multilingual-cased", cache_dir=None, device="cpu"
            )
            print(
    ...
    def predict(self, tasks, **kwargs):
        ...
        predict_dataloader = prepare_texts(
            texts, self.tokenizer, self.maxlen, SequentialSampler, self.batch_size
        )
    ...
    def fit(self, completions, workdir=None, cache_dir=None, **kwargs):
        ...
        tokenizer = BertTokenizer.from_pretrained(
            self.pretrained_model, cache_dir=cache_dir
        )

        train_dataloader = prepare_texts(
            input_texts,
            tokenizer,
            self.maxlen,
            RandomSampler,
        ...
        if self.train_logs:
            tb_writer = SummaryWriter(
                logdir=os.path.join(self.train_logs, os.path.basename(self.output_dir))
            )
        else:
            tb_writer = None
        ...
        )  # Take care of distributed/parallel training  # noqa
