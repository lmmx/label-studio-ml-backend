...
class SequenceTaggerModel(LabelStudioMLBase):
    def __init__(self, **kwargs):
        ...
        self.to_name = schema["to_name"][
            0
        ]  # this is the name of the raw data (f.e. "text")
    ...
    def convert_to_flair_annotation(self, sentence, annotations):
        ...
        for token in sentence:
            for tag in annotations[0]["result"]:
                ...
                if (start == token.start_pos) and (
                    end >= token.end_pos
                ):  # Begin token of entity
                    token.add_tag("ner", "B-" + label)
                    break
                elif (start < token.start_pos) and (
                    end >= token.end_pos
                ):  # middle token or end of entity
    ...
    def fit(self, completions, workdir=None, **kwargs):
        ...
        for compl in completions:
            sent = Sentence(
                compl["data"][self.to_name]
            )  # get raw sentence and convert to flair
    ...
    def predict(self, tasks, **kwargs):
        ...
        flair_sents = [
            Sentence(task["data"]["text"]) for task in tasks
        ]  # collect text data for each task in a list and make flair sent
