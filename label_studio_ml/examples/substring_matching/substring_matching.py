...
class SubstringMatcher(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        return [
            {"result": results, "score": avg_score / max(len(extracted_data) - 1, 1)}
        ]
    ...
    def _extract_data(data, value):
        ...
        for m in re.finditer(low, low_data):
            ...
            score = functools.reduce(
                lambda a, b: a + b, [1 if k[0] == k[1] else 0 for k in zip(value, d)]
            ) / len(d)
            temp = {"start": start, "end": start + len(low), "text": d, "score": score}
            result.append(temp)
        return result
