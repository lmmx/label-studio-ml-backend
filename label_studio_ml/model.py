...
class JobManager(object):
    ...
    def job(self, model_class, event: str, data: Dict, job_id: str):
        ...
        with self.start_run(event, data, job_id):
            model_version = data.get("model_version") or data.get("project", {}).get(
                "model_version"
            )
            job_result = self.get_result(model_version)
            label_config = data.get("label_config") or data.get("project", {}).get(
                "label_config"
            )
            train_output = job_result.get("train_output")
            logger.debug(
                f"Load model with label_config={label_config} and train_output={train_output}"
            )
...
class SimpleJobManager(JobManager):
    ...
    def _get_result_from_job_id(self, job_id):
        ...
        if not os.path.exists(job_dir):
            logger.warning(
                f"=> Warning: {job_id} dir doesn't exist. "
    ...
    def iter_finished_jobs(self):
        ...
        return reversed(
            sorted(map(int, filter(lambda d: d.isdigit(), os.listdir(self.model_dir))))
        )
...
class RQJobManager(JobManager):
    ...
    MAX_QUEUE_LEN = (
        1  # Controls a maximal amount of simultaneous jobs running in queue.
    )
    ...
    def run_job(self, model_class, args: tuple):
        ...
        if len(queue) >= self.MAX_QUEUE_LEN:
            logger.warning(
                f"Maximal RQ queue len {self.MAX_QUEUE_LEN} reached. Job is not started."
            )
            return
        job = queue.enqueue(
            self.job,
...
class LabelStudioMLManager(object):
    ...
    def has_active_model(cls, project):
        if not os.getenv(
            "LABEL_STUDIO_ML_BACKEND_V2", default=LABEL_STUDIO_ML_BACKEND_V2_DEFAULT
        ):
    ...
    def fetch(cls, project=None, label_config=None, force_reload=False, **kwargs):
        ...
        if not cls._current_model or model_version != cls._current_model.model_version:
            ...
            if job_result:
                ...
                model = cls.model_class(
                    label_config=label_config, train_output=job_result, **kwargs
                )
                cls._current_model = ModelWrapper(
    ...
    def predict(
        ...
        **kwargs,
        ...
        if not cls._current_model:
            raise ValueError(
                f"Model is not loaded for {cls.__class__.__name__}: run setup() before using predict()"
            )
